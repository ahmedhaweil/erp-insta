...
import { usePurchaseOrder, useConfirmPurchaseOrder, useCancelPurchaseOrder, useSuppliers } from '@/hooks/use-purchasing';
...
export default function PurchaseOrderDetailPage() {
  ...
  const supplierName = supplier ? (locale === 'ar' ? supplier.nameAr : (supplier.nameEn || supplier.nameAr)) : order.supplierId;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/purchasing/orders" className="text-sm text-gray-500 hover:text-gray-700">← {tc('back')}</Link>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div><p className="text-xs text-gray-500 mb-1">{tc('status')}</p><StatusBadge status={order.status} label={getStatusLabel(order.status)} /></div>
        </div>
      </div>
